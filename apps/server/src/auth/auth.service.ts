...
class AuthService {
    ...
    public async setJwtCookie(user: IUser, response: Response): Promise<IAuthResponse> {
        ...
        const access_token = this.jwtService.sign(tokenPayload, { expiresIn: ACCESS_COOKIE_LIFE_TIME })
        const refresh_token = this.jwtService.sign({}, { expiresIn: REFRESH_COOKIE_LIFE_TIME })
