...
import { IAvailableDates, ICommonStatistic, IPairStatistic, TScoreCaption } from '@tennis-stats/types';
...
class StatisticService {
  ...
  private async getInGameTime(): Promise<string> {
    const raw = await this.gameSetRepository.createQueryBuilder('gameSet')
      .select('SUM(TIMESTAMPDIFF(SECOND, gameSet.startDate, gameSet.endDate))', 'seconds')
